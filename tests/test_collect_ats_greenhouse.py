"""Greenhouse collector contract tests — blueprint/08-TESTING.md §3."""

import json
from pathlib import Path

import pytest

from factories_collect import FakeHttpSession
from jobtracker.collect.ats.greenhouse import GreenhouseCollector
from jobtracker.core.enums import Source
from jobtracker.core.errors import SourceSchemaChanged
from jobtracker.core.models import Board

REPO_ROOT = Path(__file__).resolve().parent.parent
FIXTURE = REPO_ROOT / "tests" / "fixtures" / "payloads" / "greenhouse" / "akuna_capital.json"

pytestmark = pytest.mark.contract


def _board(**overrides: object) -> Board:
    base: dict[str, object] = {
        "company_slug": "akuna_capital",
        "company_name": "Akuna Capital",
        "source": Source.GREENHOUSE,
        "token": "akunacapital",
        "sector": "prop_trading",
        "hq_country": "US",
        "priority": 1,
        "enabled": True,
    }
    base.update(overrides)
    return Board(**base)  # type: ignore[arg-type]


def test_nominal_response_maps_every_field() -> None:
    payload = json.loads(FIXTURE.read_text())
    session = FakeHttpSession(json_response=payload)
    result = GreenhouseCollector().fetch(_board(), session)

    assert result.requests_made == 1
    assert result.truncated is False
    assert len(result.postings) == len(payload["jobs"])
    first = result.postings[0]
    job = payload["jobs"][0]
    assert first.source == Source.GREENHOUSE
    assert first.company_slug == "akuna_capital"
    assert first.source_job_id == str(job["id"])
    assert first.title_raw == job["title"]
    assert first.description_raw == job["content"]
    assert first.location_raw == job["location"]["name"]
    assert first.url == job["absolute_url"]
    assert json.loads(first.payload) == job


def test_empty_board_is_zero_postings_no_exception() -> None:
    session = FakeHttpSession(json_response={"jobs": []})
    result = GreenhouseCollector().fetch(_board(), session)
    assert result.postings == ()
    assert result.truncated is False


def test_unrecognizable_payload_raises_schema_changed() -> None:
    session = FakeHttpSession(json_response={"unexpected": "shape"})
    with pytest.raises(SourceSchemaChanged):
        GreenhouseCollector().fetch(_board(), session)


def test_job_missing_a_required_field_is_skipped_run_continues() -> None:
    session = FakeHttpSession(
        json_response={
            "jobs": [
                {"id": 1, "title": "Missing URL"},
                {
                    "id": 2,
                    "title": "Fine",
                    "absolute_url": "https://x/2",
                    "location": {"name": "Paris"},
                },
            ]
        }
    )
    result = GreenhouseCollector().fetch(_board(), session)
    assert len(result.postings) == 1
    assert result.postings[0].source_job_id == "2"


def test_department_is_taken_from_the_first_entry() -> None:
    session = FakeHttpSession(
        json_response={
            "jobs": [
                {
                    "id": 1,
                    "title": "Quant Dev",
                    "absolute_url": "https://x/1",
                    "location": {"name": "London"},
                    "departments": [{"name": "Engineering"}, {"name": "Other"}],
                }
            ]
        }
    )
    result = GreenhouseCollector().fetch(_board(), session)
    assert result.postings[0].department_raw == "Engineering"
