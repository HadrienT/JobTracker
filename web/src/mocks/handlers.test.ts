...
import type { CompanyOut, FacetCounts, HealthOut, Page, PostingDetailOut, PostingOut } from './contract'
...
describe('MSW handlers — the seven routes of blueprint/03-INTERFACES.md §3.6', () => {
  it('serves at least 2000 representative postings across GET /postings', async () => {
    ...
    const res = await fetch('/api/postings?limit=100')
  ...
  it('paginates via an opaque cursor', async () => {
    const first = (await (await fetch('/api/postings?limit=50')).json()) as Page<PostingOut>
    const cursor = first.next_cursor
    expect(cursor).not.toBeNull()
    const second = (await (await fetch(`/api/postings?limit=50&cursor=${String(cursor)}`)).json()) as Page<PostingOut>
  ...
  it('GET /postings/:id returns the full detail shape', async () => {
    ...
    const res = await fetch(`/api/postings/${target.posting_id}`)
    expect(res.status).toBe(200)
  ...
  it('GET /postings/:id returns 404 for an unknown id', async () => {
    const res = await fetch('/api/postings/does-not-exist')
  ...
  it('POST /postings/:id/favorite returns 204', async () => {
    const target = POSTINGS[0]
    if (!target) throw new Error('fixture is empty')
    const res = await fetch(`/api/postings/${target.posting_id}/favorite`, {
      method: 'POST',
  ...
  it('POST /postings/:id/hide returns 204', async () => {
    const target = POSTINGS[0]
    if (!target) throw new Error('fixture is empty')
    const res = await fetch(`/api/postings/${target.posting_id}/hide`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ value: true }),
    })
  ...
  it('GET /health returns per-source status', async () => {
    ...
    expect(res.status).toBe(200)
    const body = (await res.json()) as HealthOut
